...
- 結果・知見・戦略はすべて日報 `plans/YYYYMMDD_daily.md` に集約する（個別planファイルは作らない）
...
- **日報 `plans/YYYYMMDD_daily.md` がすべての記録の中心**（セッション終了時 or 区切りで更新）
  - 1日1ファイル。戦略・ロードマップ・知見・実験結果をすべてここに書く
  - 構成: コンペ情報 → 今日やったこと → 数値まとめ → 戦略 → 判断・知見 → データ在庫 → 次にやること
