...
```json
{
  "title": "ROGII v001 expA02 super clone (single LGB)",
  "id": "<kaggle-username>/<slug>",
  "subtitle": "OOF=11.16ft / abs=11.02ft (5-fold)",
  "description": "v001 expA02_super_clone (single LGB, no ensemble). 5-fold OOF residual RMSE = 11.16 ft. 170 features, fold v1 (KMeans K=5 spatial GroupKFold), 770 train wells. Source: workspace/expA02_super_clone/results/expA02_super_clone_single.",
  "isPrivate": true,
  "licenses": [{"name": "CC0-1.0"}],
  "keywords": ["lightgbm", "geosteering"],
  "collaborators": [],
  "data": []
}
```
...
- 一部のキーワード（コンペ名そのもの "rogii" 等）は Kaggle 側で reject されるが、Dataset 自体は問題なく作成される
...
```json
{
  ...
  "title": "v001 expA02 super clone inference",
...
CV: OOF residual=X.XXft, abs=Y.YYft
"""
...
```
"title": "v003 expA04 anchor calibration (OOF=8.5ft)"
```
...
assert sub["tvt"].notna().all(), "NaN in submission"
assert sub["tvt"].between(LOW, HIGH).all(), "tvt out of range"
```
...
- `submit/v001_expA02_super_clone/` を参照
- `upload.sh`, `kernel-metadata.json`, `dataset-metadata.json` がそのまま参考になる
- `inference_notebook.py` は ~700 行で all-in-one（imputers / PF / beam / build_well / inference）
